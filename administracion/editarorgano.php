    ...
    <body>
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-9">
                    <form method="post" id="form_CREARcomposicionquimica" action="recursos/acciones.php?tarea=20&id=<?php echo $_GET["id"]; ?>">
                        <div class="col-md-12 titulopagina" style="margin-top: 125px;">Organo</div>
                    <div class="col-md-12">
                    ...
                    <?php
                        $con=Conexion();
                        $sql_ORGANO="select * from organo where idorgano='".$_GET["id"]."'";
			$result_ORGANO=mysql_query($sql_ORGANO,$con) or die(mysql_error()); 
                        if(mysql_num_rows($result_ORGANO)>0){
                            $fila = mysql_fetch_assoc($result_ORGANO);                                                            
                        }                    
                    ?>
                        ...
                        <select id="sistema" name="sistema" class="selectpicker show-tick" data-live-search="true" data-width="100%" required="required">
                            <?php
                                ...
                                if(mysql_num_rows($result_listaSISTEMA)>0){
                                    while ($organo = mysql_fetch_assoc($result_listaSISTEMA)) {
                                        if($fila["idsistema"]==$organo["idsistema"]){
                                            echo "<option selected='selected' value='".$organo["idsistema"]."'>".$organo["nombre"]."</option>";
                                        }else{
                                            echo "<option value='".$organo["idsistema"]."'>".$organo["nombre"]."</option>";
                                        }
