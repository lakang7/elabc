...
<html>
    <head>
        ...
        <title>Listar | Composición Quimica</title>        
        <script type="text/javascript" language="JavaScript" >
					...
					$result_listaCOMQUI=mysql_query($sql_listaCOMQUI,$con) or die(mysql_error()); 
                                        ...
                                        if(mysql_num_rows($result_listaCOMQUI)>0){
                                            while ($fila = mysql_fetch_assoc($result_listaCOMQUI)) {
                                                echo "<div id=\"myModal".$fila["idcomquimica"]."\" class=\"modal fade\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-header\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">&times;</button><h4 class=\"modal-title\">Confimación de Elminación</h4></div><div class=\"modal-body\"><p>¿Esta seguro de que desea elminar el elemento ".trim($fila["nombre"])."?</p></div><div class=\"modal-footer\"><button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">Cancelar</button><button type=\"button\" class=\"btn btn-primary\" onclick=eliminar(\"".$fila["idcomquimica"]."\")>Eliminar Elemento</button></div></div></div></div>";
                                                echo "<tr>";
                                                echo "<td style='width:  5%'>".$fila["idcomquimica"]."</td>";
                                                echo "<td style='width: 30%'>".$fila["nombre"]."</td>";
                                                echo "<td style='width: 40%'>".$fila["descripcion"]."</td>";
                                                echo "<td style='width: 20%'>";
                                                echo "<button type='button' onclick=redirigir('editarcomposicionquimica.php?id=".$fila["idcomquimica"]."') class='btn btn-default btn-sm'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span> Editar</button>";  
                                                echo "<button type='button' onclick=confirmar(\"".trim($fila["idcomquimica"])."\") class='btn btn-default btn-sm'><span class='glyphicon glyphicon-remove' aria-hidden='true'></span> Eliminar</button>";             
                                                echo "</td>";       
                                                echo "</tr>";                                                    
                                            }
					}	                                                                                
    ...
    <script type="text/javascript">                    
        ...
        function eliminar(id){
            location.href="recursos/acciones.php?tarea=3&id="+id;
        }
    </script>     
</html>
