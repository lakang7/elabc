...
<html>
    <head>
        ...
        <title>Listar | Terapia Alternativa</title>        
        <script type="text/javascript" language="JavaScript" >
					...
					$result_listaTERAPIA=mysql_query($sql_listaTERAPIA,$con) or die(mysql_error()); 
                                        ...
                                        if(mysql_num_rows($result_listaTERAPIA)>0){
                                            while ($fila = mysql_fetch_assoc($result_listaTERAPIA)) {
                                                echo "<div id=\"myModal".$fila["idterapia"]."\" class=\"modal fade\"><div class=\"modal-dialog\"><div class=\"modal-content\"><div class=\"modal-header\"><button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">&times;</button><h4 class=\"modal-title\">Confimación de Elminación</h4></div><div class=\"modal-body\"><p>¿Esta seguro de que desea elminar el elemento ".trim($fila["titulo"])."?</p></div><div class=\"modal-footer\"><button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">Cancelar</button><button type=\"button\" class=\"btn btn-primary\" onclick=eliminar(\"".$fila["idterapia"]."\")>Eliminar Elemento</button></div></div></div></div>";
                                                echo "<tr>";
                                                echo "<td style='width:  5%'>".$fila["idterapia"]."</td>";
                                                echo "<td style='width: 30%'>".$fila["titulo"]."</td>";
                                                echo "<td style='width: 40%'>".$fila["descripcioncatalogo"]."</td>";
                                                echo "<td style='width: 20%'>";
                                                echo "<button type='button' onclick=redirigir('editarterapiaalternativa.php?id=".$fila["idterapia"]."') class='btn btn-default btn-sm'><span class='glyphicon glyphicon-pencil' aria-hidden='true'></span> Editar</button>";  
                                                echo "<button type='button' onclick=confirmar(\"".trim($fila["idterapia"])."\") class='btn btn-default btn-sm'><span class='glyphicon glyphicon-remove' aria-hidden='true'></span> Eliminar</button>";             
                                                echo "</td>";       
                                                echo "</tr>";                                                    
                                            }
					}	                                                                                
    ...
    <script type="text/javascript">                    
        ...
        function eliminar(id){
            location.href="recursos/acciones.php?tarea=64&id="+id;
        }
    </script>    
</html>
